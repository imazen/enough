...
//! | `StopToken`               |  no | yes | no  | almost-enough |
//! | `Option<StopToken>`       |  no | yes | yes | almost-enough |
//!
...
//! | `StopRef<'a>` |  yes  | yes  |   no    | Borrowed — can't wrap in StopToken |
...
//! | StopToken       |  yes  |  no  |   yes   | Cloneable type erasure (Arc) |
...
//! | Attempt | Why |
//! |---------|-----|
//! | `StopToken::new(source.as_ref())` | StopRef not 'static |
...
use almost_enough::{
    ...
    Stopper, SyncStopper, Unstoppable,
};
...
fn accept_token(stop: StopToken) -> bool {
    stop.should_stop()
}

fn accept_option_token(stop: Option<StopToken>) -> bool {
    stop.should_stop()
}
...
fn unstoppable_to_dynstop() {
    assert!(!accept_token(Unstoppable.into_token()));
}
#[test]
fn unstoppable_to_option_dynstop() {
    assert!(!accept_option_token(Some(Unstoppable.into_token())));
}
...
fn stopper_to_dynstop() {
    assert!(!accept_token(Stopper::new().into_token()));
}
#[test]
fn stopper_to_option_dynstop() {
    assert!(!accept_option_token(Some(Stopper::new().into_token())));
}
...
fn stopsource_to_dynstop() {
    assert!(!accept_token(StopToken::new(StopSource::new())));
}
...
// StopRef CANNOT be wrapped in StopToken/BoxedStop (not 'static):
//   StopToken::new(source.as_ref())   // ERROR: lifetime
...
fn fnstop_noclone_to_dynstop() {
    // Non-Clone closure works — StopToken doesn't require Clone on T
    ...
    assert!(!accept_token(StopToken::new(f)));
}

// ═══════════════════════════════════════════════════════════════════
// StopToken → Clone + 'static — works everywhere
// ═══════════════════════════════════════════════════════════════════

#[test]
fn dynstop_to_impl() {
    assert!(!accept_impl(Stopper::new().into_token()));
}
#[test]
fn dynstop_to_impl_clone() {
    assert!(!accept_clone_stop(Stopper::new().into_token()));
}
#[test]
fn dynstop_to_dyn() {
    let s = Stopper::new().into_token();
    assert!(!accept_dyn(&s));
}
#[test]
...
fn syncstopper_to_dynstop() {
    assert!(!accept_token(StopToken::new(SyncStopper::new())));
}
#[test]
...
fn childstopper_to_dynstop() {
    assert!(!accept_token(StopToken::new(ChildStopper::new())));
}
...
// ═══════════════════════════════════════════════════════════════════
// Thread cloning: StopToken vs impl Stop + Clone
// ═══════════════════════════════════════════════════════════════════
...
mod cloning {
    ...
    fn impl_clone_to_thread() {
        ...
        assert!(!spawn_with(Stopper::new().into_token()));
    }
...
// | stop.clone() on &dyn Stop            | Clone not object-safe |
// | thread::spawn with &dyn Stop         | not 'static          |
// | accept_option_token(None)          | can't infer type     |
